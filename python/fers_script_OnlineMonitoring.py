...
def resolve_run_number(datafiles_dir: Path) -> int:
    """
    Priority:
    1) --run argument
    2) NEW_RUN_NUMBER env var
    3) interactive selection from available runs
    """
    parser = argparse.ArgumentParser(add_help=True)
    parser.add_argument("--run", type=int, default=None, help="Run number to process")
    args = parser.parse_args()

    if args.run is not None:
        return args.run

    env_run = os.getenv("NEW_RUN_NUMBER")
    if env_run:
        try:
            return int(env_run)
        except ValueError:
            print(f"[WARNING] Ignoring invalid NEW_RUN_NUMBER='{env_run}'")

    return ask_run_from_available(datafiles_dir)
...
def main():
    ...
    RUN_NUMBER = resolve_run_number(DATAFILES_DIR)
    ...
    # Fixed sampling for online monitoring: always 1%
    sampling_percentage = 1.0
    sampling_divisor = 100
    print(f"[INFO] Sampling fixed to {sampling_percentage:.1f}% (1 every {sampling_divisor} events)")
